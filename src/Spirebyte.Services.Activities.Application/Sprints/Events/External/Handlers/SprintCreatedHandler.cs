...
public class SprintCreatedHandler : IEventHandler<SprintCreated>
{
    ...
    public SprintCreatedHandler(IActivityRepository activityRepository, IHubService hubService, IContextAccessor contextAccessor)
    {
    ...
    public async Task HandleAsync(SprintCreated @event, CancellationToken cancellationToken = default)
    {
        var activity = new Activity(Guid.NewGuid(), _contextAccessor.Context.GetUserId(), Array.Empty<Guid>(), @event.ProjectId,
            ActivityType.SprintCreated, new[] { @event as object }, DateTime.Now);

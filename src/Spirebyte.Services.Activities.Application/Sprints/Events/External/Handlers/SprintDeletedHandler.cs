...
public class SprintDeletedHandler : IEventHandler<SprintDeleted>
{
    ...
    public SprintDeletedHandler(IActivityRepository activityRepository, IHubService hubService, IContextAccessor contextAccessor)
    {
    ...
    public async Task HandleAsync(SprintDeleted @event, CancellationToken cancellationToken = default)
    {
        var activity = new Activity(Guid.NewGuid(), _contextAccessor.Context.GetUserId(), Array.Empty<Guid>(), @event.ProjectId,
            ActivityType.SprintDeleted, new[] { @event as object }, DateTime.Now);

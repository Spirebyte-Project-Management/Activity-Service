...
public class SprintUpdatedHandler : IEventHandler<SprintUpdated>
{
    ...
    public SprintUpdatedHandler(IActivityRepository activityRepository, IHubService hubService, IContextAccessor contextAccessor)
    {
    ...
    public async Task HandleAsync(SprintUpdated @event, CancellationToken cancellationToken = default)
    {
        var activity = new Activity(Guid.NewGuid(), _contextAccessor.Context.GetUserId(), Array.Empty<Guid>(), @event.ProjectId,
            ActivityType.SprintUpdated, new[] { @event as object }, DateTime.Now);

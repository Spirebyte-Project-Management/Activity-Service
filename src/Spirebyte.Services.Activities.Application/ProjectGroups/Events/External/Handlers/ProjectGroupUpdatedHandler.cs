...
public class ProjectGroupUpdatedHandler : IEventHandler<ProjectGroupUpdated>
{
    private readonly IActivityRepository _activityRepository;
    private readonly IAppContext _appContext;
    private readonly IHubService _hubService;

    public ProjectGroupUpdatedHandler(IActivityRepository activityRepository, IHubService hubService, IAppContext appContext)
    {
    ...
    public async Task HandleAsync(ProjectGroupUpdated @event, CancellationToken cancellationToken = default)
    {
        var activity = new Activity(Guid.NewGuid(), _appContext.Identity.Id, Array.Empty<Guid>(), @event.ProjectId,
            ActivityType.ProjectGroupUpdated, new[] { @event as object }, DateTime.Now);
